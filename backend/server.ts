    ...
    if (useSqlPersistence) {
      ...
      for (const p of providers) {
        ...
        if (pCompute.length === 0 && compute_selections?.length > 0) {
          ...
          const sp: any = await sqlGetById("compute_pricing", source.compute_pricing_id);
          if (sp) {
            let equivalent = await sqlSelect("compute_pricing", { provider_id: p.id, vcpu: sp.vcpu, memory_gb: sp.memory_gb }, { limit: 1 });
            if (equivalent.length === 0) {
              const nearest = await findNearestComputeEquivalent(p.id, Number(sp.vcpu || 0), Number(sp.memory_gb || 0));
              if (nearest) equivalent = [nearest];
            }
            if (equivalent.length > 0) computeCost += (equivalent[0].price_per_hour || 0) * (source.quantity || 1) * 730 * modelMultiplier;
          }
        }

        if (pStorage.length === 0 && storage_selections?.length > 0) {
          const source = storage_selections[0];
          const sp: any = await sqlGetById("storage_pricing", source.storage_pricing_id);
          let basePrice = 0;
          ...
          let storageBase = 0;
          if (sp) {
            let equivalent = await sqlSelect("storage_pricing", { provider_id: p.id, storage_type: sp.storage_type }, { limit: 1 });
            if (equivalent.length === 0) {
              const nearest = await findNearestStorageEquivalent(p.id, sp.storage_type);
              if (nearest) equivalent = [nearest];
            }
            if (equivalent.length > 0) {
              basePrice = equivalent[0].price_per_gb_month || 0;
        ...
        if (pDatabase.length === 0 && database_selections?.length > 0) {
          ...
          const sp: any = await sqlGetById("database_pricing", source.database_pricing_id);
          if (sp) {
            let equivalent = await sqlSelect("database_pricing", { provider_id: p.id, db_engine: sp.db_engine }, { limit: 1 });
            if (equivalent.length === 0) {
              const nearest = await findNearestDatabaseEquivalent(p.id, sp.db_engine);
              if (nearest) equivalent = [nearest];
            }
            if (equivalent.length > 0) {
              let dbHours = 730;
              if (source.utilization_unit === "%Utilized/Month") dbHours = 730 * ((source.utilization_value || 100) / 100);
              else if (source.utilization_unit === "Hours/Day") dbHours = (source.utilization_value || 24) * 30.44;
              else if (source.utilization_unit === "Hours/Week") dbHours = (source.utilization_value || 168) * 4.345;
              else if (source.utilization_unit === "Hours/Month") dbHours = (source.utilization_value || 730);
              const dbMultiplier = source.deployment_option === "Multi-AZ" ? 2 : 1;
              const storageRate = source.storage_type === "gp3" ? 0.08 : 0.115;
              const storageC = (source.storage_gb || 0) * storageRate * dbMultiplier * (source.quantity || 1);
              databaseCost += ((equivalent[0].price_per_hour || 0) * dbMultiplier * (source.quantity || 1) * dbHours * modelMultiplier) + storageC;
            }
          }
        }
        ...
        if (pNetworking.length === 0 && networking_selections?.length > 0) {
          ...
          const sp: any = await sqlGetById("networking_pricing", source.networking_pricing_id);
          if (sp) {
            let equivalent = await sqlSelect("networking_pricing", { provider_id: p.id, service_type: sp.service_type }, { limit: 1 });
            if (equivalent.length === 0) {
              const nearest = await findNearestNetworkingEquivalent(p.id, sp.service_type);
              if (nearest) equivalent = [nearest];
            }
            if (equivalent.length > 0) networkingCost += (equivalent[0].price_per_unit || 0) * (source.quantity || 0);
          }
        }
      ...
      if (pCompute.length === 0 && compute_selections?.length > 0) {
        ...
        if (sourcePricing.exists) {
          ...
          const equivalent = await db.collection("compute_pricing")
            ...
            .get();
          if (!equivalent.empty) {
            computeCost += (equivalent.docs[0].data().price_per_hour || 0) * (source.quantity || 1) * 730 * modelMultiplier;
          }
        }
      }
      ...
      if (pDatabase.length === 0 && database_selections?.length > 0) {
        ...
        if (sourcePricing.exists) {
          ...
          if (!equivalent.empty) {
            ...
            const dbMultiplier = source.deployment_option === "Multi-AZ" ? 2 : 1;
            const storageRate = source.storage_type === "gp3" ? 0.08 : 0.115;
            const storageCost = (source.storage_gb || 0) * storageRate * dbMultiplier * (source.quantity || 1);

            databaseCost += ((equivalent.docs[0].data().price_per_hour || 0) * dbMultiplier * (source.quantity || 1) * dbHours * modelMultiplier) + storageCost;
          }
        }
      }
