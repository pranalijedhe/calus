...
import { User, AWSPricing } from "./SQLModels.ts";
...
const isMySQL = !!DATABASE_URL && (DATABASE_URL.startsWith("mysql://") || DATABASE_URL.startsWith("mariadb://"));
...
export const sequelize = (DATABASE_URL && DATABASE_URL.includes("://")) 
  ? new Sequelize(DATABASE_URL, {
      dialect: isPostgres ? "postgres" : isMySQL ? "mysql" : undefined,
      logging: false,
      models: [User, AWSPricing],
      dialectOptions: (isPostgres && DATABASE_URL.includes("sslmode=require")) ? {
        ssl: {
          require: true,
...
export async function getSQLDatabaseHealth() {
  if (!sequelize) {
    return { status: "disabled" as const, message: "DATABASE_URL is not configured." };
  }

  try {
    await sequelize.authenticate();
    return { status: "ok" as const, message: "SQL connection is healthy." };
  } catch (error) {
    return {
