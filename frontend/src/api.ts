...
export const api = {
  ...
  async deleteUser(token: string, id: string): Promise<void> {
    ...
    if (!res.ok) {
      const errorData = await res.json().catch(() => ({}));
      throw new Error(errorData.error || "Delete user failed");
    }
  },
  ...
  async getAWSInventory(token: string): Promise<any> {
    const res = await fetch(`${API_BASE}/aws/inventory`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!res.ok) {
      const errorData = await res.json().catch(() => ({}));
      throw new Error(errorData.error || "Failed to fetch AWS inventory");
    }
    return res.json();
  },
  async syncAWSPricing(token: string, region?: string): Promise<any> {
    const res = await fetch(`${API_BASE}/aws/sync-pricing`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
    ...
    if (!res.ok) throw new Error(data.error || "Failed to sync AWS pricing");
    return data;
  },
  async getAWSSyncStatus(token: string): Promise<any> {
    const res = await fetch(`${API_BASE}/aws/sync-status`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(data.error || "Failed to fetch sync status");
    return data;
  }
};
